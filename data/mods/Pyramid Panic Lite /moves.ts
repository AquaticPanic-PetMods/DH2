...
onModifyMove(move, pokemon, target) {
	...
	dragoncheer: {
		...
		condition: {
			onStart(target, source, effect) {
				...
				this.effectState.hasDragonType = target.hasType("Normal");
			},
	...
	foresight: {
		...
		condition: {
			...
			onNegateImmunity(pokemon, type) {
				if (pokemon.hasType('Psychic') && ['Normal', 'Fighting'].includes(type)) return false;
			},
	...
	saltcure: {
		...
		condition: {
			...
			onResidualOrder: 13,
			onResidual(pokemon) {
				this.damage(pokemon.baseMaxhp / (pokemon.hasType(['Water', 'Fighting']) ? 4 : 8));
			},
			onEnd(pokemon) {
	...
	smackdown: {
		...
		condition: {
			...
			onStart(pokemon) {
				let applies = false;
				if (pokemon.hasType('Normal') || pokemon.hasAbility('levitate')) applies = true;
